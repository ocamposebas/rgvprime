...
$outputPath = [IO.Path]::GetFullPath((Join-Path $outputDirectory 'rgv-prism-checkout-3.8.1.zip'))
...
try {
    ...
    if ($mainSource -notmatch 'Plugin Name:\s+RGV Storefront Card & Wallet Return' -or
        $mainSource -notmatch 'Version:\s+3\.8\.1') {
        throw 'ZIP main plugin header is not the expected 3.8.1 compact-payment build.'
    }
