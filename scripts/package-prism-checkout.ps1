...
$outputPath = [IO.Path]::GetFullPath((Join-Path $outputDirectory 'rgv-prism-checkout-3.7.3.zip'))
...
try {
    ...
    if ($mainSource -notmatch 'Plugin Name:\s+RGV Storefront Card & Wallet Return' -or
        $mainSource -notmatch 'Version:\s+3\.7\.3') {
        throw 'ZIP main plugin header is not the expected 3.7.3 wallet-gesture build.'
    }
