...
$outputPath = [IO.Path]::GetFullPath((Join-Path $outputDirectory 'rgv-storefront-card-return-2.0.0.zip'))
...
try {
    ...
    if ($mainSource -notmatch 'Plugin Name:\s+RGV Storefront Card & Wallet Return' -or
        $mainSource -notmatch 'Version:\s+2\.0\.0') {
        throw 'ZIP main plugin header is not the expected 2.0.0 card and wallet return build.'
    }
