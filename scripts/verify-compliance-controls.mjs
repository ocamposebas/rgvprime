...
const [ageGate, cart, checkout, proxy, middleware, zelle, cardWallet, orbit, orbitSecure] = await Promise.all([
...
assert(checkout.includes("/api/checkout/card-order") && checkout.includes("/api/checkout/card-wallet-order") && checkout.includes("/api/checkout/zelle-order"), "orders must use protected server routes");
...
for (const backend of [zelle, cardWallet, orbit, orbitSecure]) {
  assert(backend.toLowerCase().includes("x-rgv-compliance-secret"), "WordPress must reject calls outside the protected storefront proxy");
  for (const key of [
