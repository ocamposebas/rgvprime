...
final class Completion {
	...
	public const META_STATUS = '_psc_payment_status';
	...
	public function maybe_complete( $order, array $payment, array $guard = array() ): bool {
		...
		$order->update_meta_data( self::META_STATUS, 'succeeded' );
		$order->save();
	...
	/** Persist a verified asynchronous state without marking the Woo order paid. */
	public function accept_progress( $order, array $payment, array $guard = array() ): bool {
		$status = (string) ( $payment['status'] ?? '' );
		if ( ! is_object( $order ) || ! in_array( $status, array( 'creating', 'requires_action', 'processing', 'attention' ), true ) || ! $this->identity_matches( $order, $payment ) ) { return false; }
		$key = (string) ( $guard['claim_key'] ?? '' );
		$owner = (string) ( $guard['owner_token'] ?? '' );
		$fence = (int) ( $guard['fence'] ?? 0 );
		$order_id = (int) $order->get_id();
		$payment_id = (string) $payment['payment_id'];
		if ( $order->is_paid() || 'yes' === (string) $order->get_meta( self::META_COMPLETED, true ) ) {
			return hash_equals( (string) $order->get_meta( self::META_PAYMENT_ID, true ), $payment_id );
		}
		if ( '' === $key || '' === $owner || $fence < 1 || $order_id < 1
			|| ! Plugin::instance()->checkout_claim()->assert_bound_order( $key, $owner, $fence, $order_id ) ) { return false; }
		$fresh = function_exists( 'wc_get_order' ) ? wc_get_order( $order_id ) : false;
		if ( is_object( $fresh ) ) { $fresh->read_meta_data( true ); }
		if ( ! is_object( $fresh ) || ! $this->identity_matches( $fresh, $payment ) ) { return false; }
		if ( $fresh->is_paid() || 'yes' === (string) $fresh->get_meta( self::META_COMPLETED, true ) ) {
			return hash_equals( (string) $fresh->get_meta( self::META_PAYMENT_ID, true ), $payment_id );
		}
		$fresh->update_meta_data( self::META_PAYMENT_ID, $payment_id );
		$fresh->update_meta_data( self::META_FEE_MINOR, (int) ( $payment['fee_minor'] ?? 0 ) );
		$fresh->update_meta_data( self::META_STATUS, $status );
		if ( 'processing' === $status && ! $fresh->has_status( array( 'on-hold', 'processing', 'completed' ) ) ) {
			$fresh->set_status( 'on-hold' );
			$fresh->add_order_note( 'Payment submitted and awaiting asynchronous settlement.' );
		}
		$fresh->save();
		return true;
	}
	...
	/**
	 * Safely close a proven unpaid attempt. A real PI requires a terminal unpaid state;
	...
	public function finish_unpaid( $order, array $identity, ?array $payment = null ): bool {
		...
		try {
			...
			if ( null !== $payment ) {
				$fresh->update_meta_data( self::META_PAYMENT_ID, (string) $payment['payment_id'] );
				$fresh->update_meta_data( self::META_FEE_MINOR, (int) ( $payment['fee_minor'] ?? 0 ) );
				$fresh->update_meta_data( self::META_STATUS, (string) $payment['status'] );
			}
			$fresh->add_order_note( null === $payment ? 'PRISM confirmed no payment was created for this attempt.' : 'PRISM confirmed this payment ended without collection (' . (string) $payment['status'] . ').' );
