...
final class Rest_Routes {
	...
	public function handle_callback( $request ) {
		...
		$completion = Plugin::instance()->completion();
		$status = (string) $payment['status'];
		if ( 'succeeded' === $status ) {
			$done = $completion->maybe_complete( $order, $payment, $guard );
		} elseif ( in_array( $status, array( 'creating', 'requires_action', 'processing', 'attention' ), true ) ) {
			// Asynchronous payment methods must ACK progress without marking Woo paid.
			$done = $completion->accept_progress( $order, $payment, $guard );
		} else {
			// callback_valid() limits this branch to failed/canceled terminal states.
			$identity = Completion::recovery_identity( $order );
			$done = $completion->finish_unpaid( $order, $identity, $payment );
	...
	public function ajax_create_attempt(): void {
		...
		if ( is_array( $pending ) && ! empty( $pending['attempt_id'] ) && self::pending_matches_order( $pending, $pay_order ) ) {
		...
		// A pending confirmation belongs only to its recorded Woo order. A cart
		// checkout (no order yet), or a different order-pay URL, must not inherit it.
	...
	private static function pending_matches_order( array $pending, $order ): bool {
		return is_object( $order )
			&& method_exists( $order, 'get_id' )
			&& (int) ( $pending['order_id'] ?? 0 ) > 0
			&& (int) $pending['order_id'] === (int) $order->get_id();
	}
	public static function clear_matching_pending( int $order_id, string $attempt_id, string $operation_id ): bool {
		if ( $order_id < 1 || '' === $attempt_id || '' === $operation_id || ! function_exists( 'WC' ) || ! WC()->session ) { return false; }
		$pending = WC()->session->get( 'psc_confirm_pending' );
		if ( ! is_array( $pending )
			|| (int) ( $pending['order_id'] ?? 0 ) !== $order_id
			|| ! hash_equals( (string) ( $pending['attempt_id'] ?? '' ), $attempt_id )
			|| ! hash_equals( (string) ( $pending['operation_id'] ?? '' ), $operation_id ) ) {
			return false;
		}
	...
	public static function clear_matching_recovery( array $identity ): void {
		...
		$cleared = self::clear_matching_pending( (int) $identity['order_id'], (string) $identity[ Completion::META_ATTEMPT_ID ], (string) $identity['_psc_confirmation_operation_id'] );
	...
	private function callback_valid( $data ): bool {
		...
		if ( ! is_array( $data ) || array_diff( $top, array_keys( $data ) ) || ! preg_match( '/^[0-9a-f-]{36}$/i', (string) $data['outbox_id'] ) || ! preg_match( '/^[0-9a-f-]{36}$/i', (string) $data['site_id'] ) || ! preg_match( '/^[a-f0-9]{64}$/', (string) $data['woo_order_key_hash'] ) || ! is_int( $data['woo_order_id'] ) || $data['woo_order_id'] < 1 || ! is_array( $data['payment'] ) ) {
			return false;
		}
		$required = array( 'payment_id', 'attempt_id', 'connected_account', 'status', 'amount_minor', 'fee_minor', 'currency' );
		$p = $data['payment'];
		return ! array_diff( $required, array_keys( $p ) )
			...
			&& ( ! array_key_exists( 'application_fee_id', $p ) || null === $p['application_fee_id'] || is_string( $p['application_fee_id'] ) )
			&& ( ! array_key_exists( 'client_secret', $p ) || null === $p['client_secret'] || is_string( $p['client_secret'] ) )
			&& ( ! array_key_exists( 'next_action_required', $p ) || is_bool( $p['next_action_required'] ) );
	}
