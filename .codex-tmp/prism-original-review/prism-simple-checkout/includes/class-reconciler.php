...
final class Reconciler {
	...
	public function holds_order( $order ): bool {
		...
		return $this->age_seconds( $order ) <= (int) apply_filters( 'psc_reconcile_hold_window', self::HOLD_WINDOW, $order );
	}
	 ...
	 *
	 * Two populations:
	 *  - wc-pending PRISM orders still holding a payment id (the live hold), and
	...
	private function candidates(): array {
		...
		$sets = array(
			'pending' => array( 'status' => array( 'wc-pending' ) ),
	...
	private function needs_reconcile( $order ): bool {
		...
		return (int) $order->get_meta( self::META_ATTEMPTS, true ) < (int) apply_filters( 'psc_reconcile_max_attempts', self::MAX_ATTEMPTS, $order );
	}
	...
	private function maybe_give_up( $order ): void {
		...
		$max      = (int) apply_filters( 'psc_reconcile_max_attempts', self::MAX_ATTEMPTS, $order );
		if ( $attempts < $max ) {
			return;
		}
