...
final class RGV_Storefront_Card_Wallet_Return {
  ...
  public function enqueue_storefront_checkout() {
    ...
    // Keep the provider's native Express Checkout for eligible wallets while
    // limiting the regular Payment Element to card and US bank account.
    // Run before the provider initializes Elements so there is no method reflow.
    $card_only_script = <<<'JS'
(function () {
      ...
      var guardedCreate = function (type, elementOptions) {
        if (type === 'expressCheckout') {
          return originalCreate(type, Object.assign({}, elementOptions || {}, {
            ...
            paymentMethods: Object.assign({}, elementOptions && elementOptions.paymentMethods || {}, {
              applePay: 'auto',
              googlePay: 'auto',
              link: 'auto',
              amazonPay: 'never',
            ...
            layout: Object.assign({}, elementOptions && elementOptions.layout || {}, {
              maxColumns: 2,
              overflow: 'never'
            })
          }));
...
JS;
  ...
  public function neutralize_frontend_branding() {
    ...
    // Stripe account. Keep native Express wallets and constrain the deferred
    // Payment Element to card and US bank account. The server-side confirmation
...
CSS;
  ...
  private function payment_page_premium_css() {
    return <<<'CSS'
      ...
      body.rgv-card-wallet-payment-page form#order_review.rgv-payment-layout-ready :is(#psc-checkout-root, #psc-checkout) #psc-card-toggle {
        display: none !important;
      }

      body.rgv-card-wallet-payment-page form#order_review.rgv-payment-layout-ready :is(#psc-checkout-root, #psc-checkout) :is(#psc-wallet-stage, .psc-wallet-stage):not([hidden]) {
        width: 100% !important;
        min-width: 0;
        margin: 0 0 16px !important;
      }
