...
final class Rest_Routes {
	...
	public function handle_callback( $request ) {
		...
		$current_attempt = (string) $order->get_meta( Completion::META_ATTEMPT_ID, true );
		$tombstone = Completion::terminal_attempt( $order, $incoming_attempt );
		if ( null !== $tombstone ) {
			if ( 'succeeded' === $status ) {
				return $this->callback_conflict_hold( $order, $payment, 'late_success_after_terminal', 'late_terminal_success' );
			}
			$tombstone_status = (string) ( $tombstone['status'] ?? '' );
			$tombstone_payment = (string) ( $tombstone['payment_id'] ?? '' );
			$incoming_payment = (string) ( $payment['payment_id'] ?? '' );
			$identity_ok = ( empty( $tombstone['connected_account'] ) || hash_equals( (string) $tombstone['connected_account'], (string) ( $payment['connected_account'] ?? '' ) ) )
				&& ( empty( $tombstone['currency'] ) || hash_equals( strtolower( (string) $tombstone['currency'] ), strtolower( (string) ( $payment['currency'] ?? '' ) ) ) )
				&& ( empty( $tombstone['amount_minor'] ) || (int) $tombstone['amount_minor'] === (int) ( $payment['amount_minor'] ?? 0 ) );
			$real_tombstone_payment = '' !== $tombstone_payment && ! str_starts_with( $tombstone_payment, 'pending_' );
			$same_payment = $real_tombstone_payment && '' !== $incoming_payment && hash_equals( $tombstone_payment, $incoming_payment );
			if ( ! $identity_ok || ( $real_tombstone_payment && ! $same_payment ) ) {
				return $this->callback_conflict_hold( $order, $payment, 'terminal_identity_mismatch', 'terminal_identity_mismatch' );
			}
			// A canonical failed/canceled state makes an earlier non-success event
			// for that same payment stale and safe to ACK. A 404/legacy tombstone
			// may ACK only a terminal failed/canceled event; any live intermediate
			// state is new evidence that a payment exists and must stop retries.
			if ( ( in_array( $tombstone_status, array( 'failed', 'canceled' ), true ) && $same_payment )
				|| in_array( $status, array( 'failed', 'canceled' ), true ) ) {
				if ( ! in_array( $tombstone_status, array( 'failed', 'canceled' ), true )
					&& in_array( $status, array( 'failed', 'canceled' ), true ) ) {
					$this->remember_callback_terminal( $order, $payment );
				}
				return new \WP_REST_Response( array( 'ok' => true ), 200 );
			}
			return $this->callback_conflict_hold( $order, $payment, 'live_event_after_terminal', 'terminal_state_conflict' );
		}
		if ( ! hash_equals( $current_attempt, $incoming_attempt ) ) {
			if ( in_array( $status, array( 'failed', 'canceled' ), true ) ) {
				$this->remember_callback_terminal( $order, $payment );
				return new \WP_REST_Response( array( 'ok' => true ), 200 );
			}
			return $this->callback_conflict_hold( $order, $payment, 'live_attempt_mismatch', 'live_attempt_mismatch' );
		}
		...
		Service_Client::observe_fee( $payment, $order, 'callback' );
		$guard = $this->guard_for_order( $order );
		$guard['outbox_id'] = (string) $data['outbox_id'];
		if ( 'succeeded' === $status ) {
			$done = Plugin::instance()->completion()->maybe_complete( $order, $payment, $guard );
			return new \WP_REST_Response( array( 'ok' => (bool) $done ), $done ? 200 : 409 );
		}
		if ( in_array( $status, array( 'failed', 'canceled' ), true ) ) {
			$identity = Completion::recovery_identity( $order );
			$done = Plugin::instance()->completion()->finish_unpaid( $order, $identity, $payment );
			if ( $done ) { self::clear_matching_recovery( $identity ); }
			return new \WP_REST_Response( array( 'ok' => (bool) $done ), $done ? 200 : 409 );
		}
		// Persist the real payment id under the same fenced claim before ACKing an
		// intermediate event. Poll/reconciliation must never keep querying the
		// provisional pending_<attempt> marker written before confirm.
		$accepted = Plugin::instance()->completion()->accept_progress( $order, $payment, $guard );
		return new \WP_REST_Response( array( 'ok' => (bool) $accepted ), $accepted ? 200 : 409 );
	}
	private function callback_conflict_hold( $order, array $payment, string $reason, string $error ) {
		$attempt_id = (string) ( $payment['attempt_id'] ?? '' );
		$existing = $order->get_meta( Gateway::ORDER_CONFLICT_HOLD_META, true );
		$already_recorded = is_array( $existing )
			&& hash_equals( (string) ( $existing['attempt_id'] ?? '' ), $attempt_id )
			&& hash_equals( (string) ( $existing['reason'] ?? '' ), $reason );
		if ( ! $already_recorded ) {
			$order->update_meta_data(
				Gateway::ORDER_CONFLICT_HOLD_META,
				array(
			...
			$order->add_order_note( 'Conflicting signed payment states were received. Manual review is required before accepting another payment.' );
			$order->save();
		}
		return new \WP_REST_Response( array( 'error' => $error, 'ok' => false ), 409 );
	}
	private function remember_callback_terminal( $order, array $payment ): void {
		Completion::remember_terminal_attempt(
			$order,
			array(
				Completion::META_ATTEMPT_ID => (string) ( $payment['attempt_id'] ?? '' ),
				Completion::META_PAYMENT_ID => (string) ( $payment['payment_id'] ?? '' ),
				Completion::META_ACCOUNT    => (string) ( $payment['connected_account'] ?? '' ),
				Completion::META_AMOUNT_MINOR => (string) ( $payment['amount_minor'] ?? 0 ),
				Completion::META_CURRENCY   => (string) ( $payment['currency'] ?? '' ),
			),
			$payment
		);
		$order->save();
	...
	public function ajax_create_attempt(): void {
		...
		// A session can retain an ambiguous marker from a different order. Never
		// let that unrelated order block a fresh, explicitly addressed order-pay
		// request; detach only local session state and leave the old order intact
		// for its own callback/reconciler.
		if ( $pay_order && is_array( $pending )
			&& (int) ( $pending['order_id'] ?? 0 ) !== (int) $pay_order->get_id() ) {
			$stale_order_id = (int) ( $pending['order_id'] ?? 0 );
			WC()->session->set( 'psc_confirm_pending', null );
			$recovery = WC()->session->get( 'psc_recovery' );
			if ( is_array( $recovery ) && (int) ( $recovery['orderId'] ?? 0 ) === $stale_order_id ) {
		...
		if ( $current = $plugin->attempt_state()->get_attempt() ) {
			...
			$service_order_bound = ! $pay_order || (
				'v1' === (string) ( $current['service_order_binding'] ?? '' )
				&& (int) ( $current['woo_order_id'] ?? 0 ) === (int) $pay_order->get_id()
			);
			if ( $same_fp && $same_amount && $same_currency && $service_order_bound && hash_equals( (string) ( $current['research_record_id'] ?? '' ), (string) $research['id'] ) && hash_equals( (string) ( $current['research_record_digest'] ?? '' ), (string) $research['digest'] ) ) {
		...
		$service_attempt = array(
			...
			'woo_session_id'    => $session,
			'email'             => $email,
			'verification'      => $verification,
			'legal'             => $legal,
			'research_record'   => array( 'id' => $research['id'], 'digest' => $research['digest'] ),
			'cart_fingerprint'  => $fp,
			'amount_minor'      => $amount,
			'currency'          => (string) $payload['currency'],
			'return_url'        => $pay_order ? $pay_order->get_checkout_payment_url() : wc_get_checkout_url(),
		);
		// The service must bind an order-pay attempt to its Woo order when the
		// attempt is created. Sending this identity only at /confirm is too late:
		// the service cannot safely create/reconcile the payment outbox and the
		// client is left with a provisional pending_<attempt> marker forever.
		if ( $pay_order ) {
			$service_attempt['woo_order_id']  = (int) $pay_order->get_id();
			$service_attempt['woo_order_key'] = (string) $pay_order->get_order_key();
		}
		$result = $plugin->service_client()->create_checkout_attempt( $service_attempt );
		...
		$attempt = array_merge(
			...
			array(
				...
				'service_order_binding' => $pay_order ? 'v1' : '',
	...
	private function callback_valid( $data ): bool {
		...
		$top_valid = Service_Client::validate_schema(
			$data,
			array( 'outbox_id' => 'string', 'site_id' => 'string', 'woo_order_id' => 'int', 'woo_order_key_hash' => 'string', 'payment' => 'array' )
		);
		if ( is_wp_error( $top_valid )
			|| ! preg_match( '/^[0-9a-f-]{36}$/i', (string) $data['outbox_id'] )
			|| ! preg_match( '/^[0-9a-f-]{36}$/i', (string) $data['site_id'] )
			|| ! preg_match( '/^[a-f0-9]{64}$/', (string) $data['woo_order_key_hash'] )
			|| $data['woo_order_id'] < 1 ) {
			return false;
		}
		$p = $data['payment'];
		$payment_valid = Service_Client::validate_schema(
			$p,
			array( 'payment_id' => 'string', 'attempt_id' => 'string', 'connected_account' => 'string', 'status' => 'string', 'amount_minor' => 'int', 'fee_minor' => 'int', 'currency' => 'string' ),
			array( 'application_fee_id' => 'nullable_string', 'client_secret' => 'nullable_string', 'next_action_required' => 'bool' )
		);
		return ! is_wp_error( $payment_valid )
