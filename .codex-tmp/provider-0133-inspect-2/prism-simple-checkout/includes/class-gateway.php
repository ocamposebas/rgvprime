...
class Gateway extends \WC_Payment_Gateway {
	...
	public function process_payment( $order_id ) {
		...
		if ( ! hash_equals( $previous_attempt, $attempt_id ) ) {
			...
			$legacy_terminal = (string) $order->get_meta( '_psc_terminal_attempt_id', true );
			if ( '' !== $previous_attempt && hash_equals( $legacy_terminal, $previous_attempt ) ) {
				Completion::remember_terminal_attempt( $order, Completion::recovery_identity( $order ), null );
			}
		...
		// A late success from a resolved earlier attempt can place the order on
		// conflict hold while this network request is in flight. Never persist or
		// complete the new response from a stale in-memory WC_Order instance.
		$order = wc_get_order( (int) $order->get_id() );
		if ( ! $order ) {
		...
		if ( $order->get_meta( self::ORDER_CONFLICT_HOLD_META, true ) ) {
			...
			return $this->failure( 'This order requires payment review before another attempt.' );
		}
	...
	private function handle_confirm_error( $result, $order, array $attempt, $plugin, string $attempt_id, string $operation ): array {
		...
		$released = $plugin->completion()->finish_unpaid( $order, Completion::recovery_identity( $order ) ) ? 'released' : 'busy';
		if ( 'busy' === $released ) {
			if ( function_exists( 'WC' ) && WC()->session ) {
				WC()->session->set(
					'psc_confirm_pending',
		...
		// released or gone: safe clean reset (existing lines, unchanged order).
		if ( function_exists( 'WC' ) && WC()->session ) {
			WC()->session->set( 'psc_confirm_pending', null );
			WC()->session->set( 'psc_blocks_payment_data', null );
		}
